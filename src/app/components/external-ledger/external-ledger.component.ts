...
export class ExternalLedgerComponent implements OnInit {
  ...
  async addEntry(): Promise<void> {
    ...
    try {
      const result = await this.ledgerService.addEntry(newEntry);
      if (result) {
        this.loadData(this.account.id); // Reload all data
      } else {
        alert('Failed to add entry.');
      ...
      alert(`Error adding entry: ${error.message || 'Unknown error occurred.'}`);
    }
  }
  ...
  async confirmDeleteEntry(): Promise<void> {
    if (this.entryToDelete && this.account) {
      try {
        await this.ledgerService.deleteEntry(this.entryToDelete.id);
        this.loadData(this.account.id); 
      } catch (error: any) {
        alert(`Error deleting entry: ${error.message || 'Unknown error occurred.'}`);
      } finally {
        this.cancelDeleteEntry();
      }
    }
